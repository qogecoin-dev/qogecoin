# Bootstrappable Qogecoin Core Builds

This directory contains the files necessary to perform bootstrappable Qogecoin
Core builds.
...
To build Qogecoin Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Qogecoin Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/qogecoin-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/qogecoin-core/guix.sigs).
