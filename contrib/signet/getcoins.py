...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing qogecoin-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='qogecoin-cli', help='qogecoin-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Qogecoin address to which the faucet should send')
...
parser.add_argument('qogecoin_cli_args', nargs='*', help='Arguments to pass on to qogecoin-cli (default: -signet)')

args = parser.parse_args()

if args.qogecoin_cli_args == []:
    args.qogecoin_cli_args = ['-signet']


def qogecoin_cli(rpc_command_and_params):
    argv = [args.cmd] + args.qogecoin_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = qogecoin_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = qogecoin_cli(['getnewaddress', 'faucet', 'bech32'])
