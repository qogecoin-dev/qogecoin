...
void QogecoinApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the QogecoinApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", QogecoinGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void QogecoinApplication::createPaymentServer()
{
...
{
    window = new QogecoinGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &QogecoinGUI::quitRequested, this, &QogecoinApplication::requestShutdown);
...
void QogecoinApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
    ...
    m_splash->show();
    connect(this, &QogecoinApplication::splashFinished, m_splash, &SplashScreen::finish);
    connect(this, &QogecoinApplication::requestedShutdown, m_splash, &QWidget::close);
}

void QogecoinApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool QogecoinApplication::baseInitialize()
{
    return node().baseInitialize();
}

void QogecoinApplication::startThread()
{
    assert(!m_executor);
...
void QogecoinApplication::parameterSetup()
{
...
void QogecoinApplication::requestShutdown()
{
...
void QogecoinApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
...
void QogecoinApplication::handleRunawayException(const QString &message)
{
...
void QogecoinApplication::handleNonFatalException(const QString& message)
{
...
WId QogecoinApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool QogecoinApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", QogecoinGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
