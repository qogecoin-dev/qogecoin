...
QogecoinGUI::QogecoinGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
...
void QogecoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Qogecoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and qogecoin: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &QogecoinGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &QogecoinGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &QogecoinGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Qogecoin addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Qogecoin addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Qogecoin Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Qogecoin Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a qogecoin: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Qogecoin command-line options").arg(PACKAGE_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &QogecoinGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &QogecoinGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &QogecoinGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &QogecoinGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &QogecoinGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &QogecoinGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const std::pair<const std::string, bool>& i : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &QogecoinGUI::setCurrentWallet);
                    activity->open(path);
                });
            }
        ...
        connect(m_create_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &CreateWalletActivity::created, this, &QogecoinGUI::setCurrentWallet);
            activity->create();
        });
        connect(m_close_all_wallets_action, &QAction::triggered, [this] {
            m_wallet_controller->closeAllWallets(this);
        });
        connect(m_mask_values_action, &QAction::toggled, this, &QogecoinGUI::setPrivacy);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &QogecoinGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &QogecoinGUI::showDebugWindow);
}
...
void QogecoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &QogecoinGUI::setCurrentWalletBySelectorIndex);
...
void QogecoinGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &QogecoinGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &QogecoinGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &QogecoinGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &QogecoinGUI::showProgress);
...
#ifdef ENABLE_WALLET
void QogecoinGUI::setWalletController(WalletController* wallet_controller)
{
...
WalletController* QogecoinGUI::getWalletController()
{
    return m_wallet_controller;
}

void QogecoinGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &QogecoinGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &QogecoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &QogecoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &QogecoinGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &QogecoinGUI::incomingTransaction);
    connect(this, &QogecoinGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
    wallet_view->setPrivacy(isPrivacyModeActivated());
    const QString display_name = walletModel->getDisplayName();
    m_wallet_selector->addItem(display_name, QVariant::fromValue(walletModel));
}

void QogecoinGUI::removeWallet(WalletModel* walletModel)
{
    if (!walletFrame) return;
...
void QogecoinGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame) return;
...
void QogecoinGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void QogecoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void QogecoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void QogecoinGUI::createTrayIcon()
{
...
void QogecoinGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void QogecoinGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void QogecoinGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void QogecoinGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void QogecoinGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void QogecoinGUI::openClicked()
{
...
void QogecoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void QogecoinGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void QogecoinGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void QogecoinGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void QogecoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void QogecoinGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void QogecoinGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void QogecoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Qogecoin network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void QogecoinGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void QogecoinGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void QogecoinGUI::updateHeadersSyncProgressLabel()
{
...
void QogecoinGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
...
void QogecoinGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void QogecoinGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &QogecoinGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &QogecoinGUI::show);
                e->ignore();
            }
        }
    }
...
void QogecoinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void QogecoinGUI::incomingTransaction(const QString& date, QogecoinUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(QogecoinUnits::formatWithUnit(unit, amount, true));
...
void QogecoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void QogecoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool QogecoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool QogecoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void QogecoinGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void QogecoinGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void QogecoinGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void QogecoinGUI::updateProxyIcon()
{
    std::string ip_port;
...
void QogecoinGUI::updateWindowTitle()
{
...
void QogecoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void QogecoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void QogecoinGUI::detectShutdown()
{
...
void QogecoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void QogecoinGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(QogecoinGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = QogecoinGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void QogecoinGUI::subscribeToCoreSignals()
{
...
void QogecoinGUI::unsubscribeFromCoreSignals()
{
...
bool QogecoinGUI::isPrivacyModeActivated() const
{
      ...
      m_platform_style{platformStyle}
{
    ...
    QList<QogecoinUnit> units = QogecoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const QogecoinUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, QogecoinUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const QogecoinUnit u : QogecoinUnits::availableUnits()) {
        menu->addAction(QogecoinUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(QogecoinUnit newUnits)
{
    setText(QogecoinUnits::longName(newUnits));
}
