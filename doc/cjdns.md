# CJDNS support in Qogecoin Core

It is possible to run Qogecoin Core over CJDNS, an encrypted IPv6 network that
...
done outside of Qogecoin Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Qogecoin network and individual nodes.
...
before it will work with your Qogecoin Core node. This is described in steps
...
## Run Qogecoin Core with CJDNS

Once you are connected to the CJDNS network, the following Qogecoin Core
...
When enabled, this option tells Qogecoin Core that it is running in an
...
IPv6 local network. This helps Qogecoin Core perform better address management:
...
CJDNS support was added to Qogecoin Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `qogecoin-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/qogecoin/qogecoin/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Qogecoin Core

There are several ways to see your CJDNS address in Qogecoin Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `qogecoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `qogecoin-cli getpeerinfo`).
