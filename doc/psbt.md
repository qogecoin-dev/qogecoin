# PSBT Howto for Qogecoin Core

Since Qogecoin Core 0.17, an RPC interface exists for Partially Signed Qogecoin
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/qogecoin/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for Qogecoin transactions that are not fully signed
...
#### Multisig with multiple Qogecoin Core instances
...
Qogecoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `qogecoin-cli` is used.
...
Later, when *V* Qoge has been received on *Amulti*, and Bob and Carol want to
