...
These tools must be installed in order to build Qogecoin Core from source.
...
### 4. Clone Qogecoin repository

`git` should already be installed by default on your system.
Now that all the required dependencies are installed, let's clone the Qogecoin Core repository to a directory.
...
Qogecoin Core includes a GUI built with the cross-platform Qt Framework.
...
You can deploy a `.dmg` containing the Qogecoin Core application using `make deploy`.
...
Run the following in your terminal to compile Qogecoin Core:
...
``` bash
/Users/${USER}/Library/Application Support/Qogecoin/
```
