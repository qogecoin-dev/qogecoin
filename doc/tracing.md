# User-space, Statically Defined Tracing (USDT) for Qogecoin Core

Qogecoin Core includes statically defined tracepoints to allow for more
...
**Note**: Qogecoin Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Qogecoin Core
...
Multiple tools can list the available tracepoints in a `qogecoind` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Qogecoin Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/qogecoind
stap net        outbound_message 0x0000000000107c05 /src/qogecoind
stap validation block_connected  0x00000000002fb10c /src/qogecoind
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Qogecoin Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/qogecoind | grep NT_STAPSDT -A 4 -B 2
