...
Below are some notes on how to build Qogecoin Core for Windows.

The options known to work for building Qogecoin Core on Windows are:
...
Note that for WSL the Qogecoin Core source path MUST be somewhere in the default mount file system, for
example /usr/src/qogecoin, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\qogecoin`, for example:

    make install DESTDIR=/mnt/c/workspace/qogecoin
